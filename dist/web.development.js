...
function update(data) {
  ...
  underscore__WEBPACK_IMPORTED_MODULE_1___default().each(data, (_ref3) => {let { onyxMethod, key, value } = _ref3;
    ...
    const existingPromise = underscore__WEBPACK_IMPORTED_MODULE_1___default().find(queue, (item) => item.key === key);

    // If we found one, we merge with the existing update, otherwise we add a new one
    if (existingPromise) {
      existingPromise.value = underscore__WEBPACK_IMPORTED_MODULE_1___default().defaults(value, existingPromise.value);
    } else {
      ...
      const pendingUpdate = {
        ...
        promise: Promise.resolve().then(() => {
          ...
          queue = underscore__WEBPACK_IMPORTED_MODULE_1___default().reject(queue, (item) => item.key === key);
        })
      };
  ...
  return Promise.all(underscore__WEBPACK_IMPORTED_MODULE_1___default().map(queue, (item) => item.promise));
}
